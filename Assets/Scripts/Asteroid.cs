using System;
using System.Collections; using System.Collections.Generic; using UnityEngine;  [RequireComponent(typeof(SpawnableObject))] public class Asteroid : DestructableEntity {

    public SpawnableObject objectToSplitInto;     public int minPiecesCount;     public int maxPiecesCount;     public float minExplosionForce;     public float maxExplosionForce;     public float splitDistance;
    public float maxSpawnAngleDeviationFactor;

	public List<String> canBeDestroyedBy;      public int oxygenValue;
 ...
 // Oxygen Value of asteroid      // Use this for initialization     new void Start () {         base.Start(); 	} 	 	// Update is called once per frame 	new void Update () {
        base.Update();
    }      public void Split(Vector2 normal) {         Debug.Log(GetComponent<SpawnableObject>().GetSpawner().name);         if (objectToSplitInto) SplitIntoObjects(normal, objectToSplitInto, minPiecesCount, maxPiecesCount);              }      protected void SplitIntoObjects(Vector2 normal, SpawnableObject obj, int minPiecesCount, int maxPiecesCount) {         var piecesCount = UnityEngine.Random.Range(minPiecesCount, maxPiecesCount+1);         var initialDirection = Quaternion.Euler(0, 0, 90) * normal;         for (int i=0; i < piecesCount; i++) {
